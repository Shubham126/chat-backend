...
app.use(cors({
    origin: function (origin, callback) {
        ...
        // Allow localhost, 127.0.0.1, and local network IPs (192.168.x.x) in any port
...
app.use((req, res, next) => {
    ...
    res.send = function (data) {
        if (res.statusCode >= 400) {
            console.log(`[${timestamp}] "${req.method} ${req.url}" Error (${res.statusCode}): "${res.statusMessage || 'Not found'}"`);
        }
...
async function startServer() {
    try {
        ...
        app.listen(PORT, () => {
            ...
            console.log(`   📁 Frontend: Moved to separate directory`);
